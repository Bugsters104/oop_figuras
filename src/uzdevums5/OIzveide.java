...
public class OIzveide {
	...
	static int cPIzvele() {
		...
		IzveidotieObjekti.izvaditCentru(Figuras.centraObjekti), 
		0, Figuras.centraObjekti.size()-1); 
	...
	static void izveidotObjektu() {
		...
		case 0:
			...
			if((x == -1 || y == -1) && MinkuTante.ievade == null) {
				...
				"Centra punkts netika izveidots!", "Brīdinājums", 
		...
		case 1:
			...
			case "Noklusējuma":
				...
				JOptionPane.showMessageDialog(null,
				"Noklusējuma četrstūris izveidots!", "Paziņojums", 
				JOptionPane.INFORMATION_MESSAGE);
				break;
			...
			case "Centra punkts un malas":
				...
				if (p == -1 || a == -1) {
					JOptionPane.showMessageDialog(null,
					"Četrstūris netika izveidots!", "Brīdinājums", 
					JOptionPane.WARNING_MESSAGE);
					break;
				}
				Figuras.cetrsturaObjekti.add(new Cetrsturis(
				...
				JOptionPane.showMessageDialog(null,
				"Četrstūris izveidots!", "Paziņojums", 
				JOptionPane.INFORMATION_MESSAGE);
				break;
			}
			break;
		...
		case 3:
			...
			int m1 = MinkuTante.skaitlaParbaude(
					"Ievadi pirmās malas garumu", 1, 100);
			int m2 = MinkuTante.skaitlaParbaude(
					"Ievadi otrās malas garumu", 1, 100);
			int m3 = MinkuTante.skaitlaParbaude(
					"Ievadi trešās malas garumu", 1, 100);
			
			if (m1 == -1 || m2 == -1 || m3 == -1)
				break;
			
			Figuras.trijsturaObjekti.add(
			new Trijsturis(Figuras.centraObjekti.get(cPNr), m1, m2, m3));
			
			JOptionPane.showMessageDialog(null,
			"Trijstūris izveidots!", "Paziņojums", 
			JOptionPane.INFORMATION_MESSAGE);
			break;
